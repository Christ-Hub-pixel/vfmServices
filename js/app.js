...
const VFMCart = {
  ...
  bindEvents() {
    document.addEventListener('click', (e) => {
      ...
      // 1. Clic sur n'importe quel bouton de commande (universel et infaillible)
      const cardBtn = e.target.closest('.btn-add-devis-pill, .btn-add-cart, #addToCartBtn, #productPageAddToCartBtn, .product-card__footer button, .product-card__footer a, button');
      if (cardBtn) {
        const txt = cardBtn.textContent.toUpperCase();
        const isCommandBtn = cardBtn.classList.contains('btn-add-devis-pill') || 
                             cardBtn.classList.contains('btn-add-cart') || 
                             cardBtn.id === 'addToCartBtn' || 
                             cardBtn.id === 'productPageAddToCartBtn' || 
                             cardBtn.closest('.product-card__footer') || 
                             txt.includes('COMMANDER') || 
                             txt.includes('DEVIS');

        if (isCommandBtn) {
          e.preventDefault();
          const card = cardBtn.closest('.product-card, .product-detail-card, .produit-container, article, main, body');
          
          let title = 'Matériel Pedrollo VFM';
          let img = 'assets/logo.png';
          let cat = 'Équipement VFM';

          if (card) {
            title = card.querySelector('.product-card__title, #productTitle, .product-title, h1, h2, h3')?.textContent.trim() || title;
            img = card.querySelector('.product-card__image, #mainProductImg, .product-main-img, img')?.getAttribute('src') || img;
            cat = card.querySelector('.product-card__category, #productCategoryBadge, .product-category')?.textContent.trim() || cat;
          }
